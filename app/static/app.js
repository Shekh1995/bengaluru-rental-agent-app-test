document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderProperties(properties) {
    ...
    currentProperties.forEach((property) => {
      ...
      const meta = document.createElement('div'); meta.className = 'meta-info'; appendText(meta, 'p', `${property.furnishing} · ${property.built_up_sqft || 'Area not listed'} sq ft`); appendText(meta, 'p', `Metro: ${property.nearest_metro} (${property.metro_distance_km} km)`); const facts = [property.bathrooms ? `${property.bathrooms} bath` : null, property.age_in_years !== null && property.age_in_years !== undefined ? `${property.age_in_years} yrs old` : null, property.price_per_sqft ? `${money(property.price_per_sqft)}/sq ft` : null, property.is_verified ? 'Verified' : null].filter(Boolean); if (facts.length) appendText(meta, 'p', facts.join(' · ')); const water = document.createElement('p'); const waterLabel = document.createElement('strong'); waterLabel.textContent = 'Water reliability: '; water.append(waterLabel, `${property.locality_metrics.water_score}/5 · ${property.locality_metrics.green_cover} greenery`); meta.appendChild(water); card.appendChild(meta);
